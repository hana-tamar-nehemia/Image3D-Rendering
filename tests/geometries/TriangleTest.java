...
class TriangleTest {
    ...
    public void testfindIntersectionsRay() {
        ...
        assertEquals(List.of(new Point3D(1d / 3, 1d / 3, 1d / 3)), tr.findIntsersections(ray),
                "Bad intersection");

        // TC02: Against edge
        ray = new Ray(new Point3D(0, 0, -1), new Vector(1, 1, 0));
        assertEquals(List.of(new Point3D(1, 1, -1)), pl.findIntsersections(ray),
        ...
        assertEquals(List.of(new Point3D(-0.5, -0.5, 2)), pl.findIntsersections(ray),
                "Wrong intersection with plane");
        assertNull(tr.findIntsersections(ray), "Bad intersection");
        ...
        assertEquals(List.of(new Point3D(0, 1, 0)), pl.findIntsersections(ray),
                "Wrong intersection with plane");
        assertNull(tr.findIntsersections(ray), "Bad intersection");

        // TC12: On edge
        ray = new Ray(new Point3D(-1, -1, 0), new Vector(1, 1, 0));
        assertEquals(List.of(new Point3D(0.5, 0.5, 0)), pl.findIntsersections(ray),
                "Wrong intersection with plane");
        assertNull(tr.findIntsersections(ray), "Bad intersection");

        // TC13: On edge continuation
        ray = new Ray(new Point3D(-2, 0, 0), new Vector(1, 1, 0));
        assertEquals(List.of(new Point3D(-0.5, 1.5, 0)), pl.findIntsersections(ray),
                "Wrong intersection with plane");
        assertNull(tr.findIntsersections(ray), "Bad intersection");
    }
}